...
def detect_bursts(
    ...
    """
    n_win = max(1, int(round(win_s * fs)))
    ...
    min_len = max(8, int(round(min_dur_s * fs)))
    bursts = []
    for a, b in merged:
        if b - a < min_len:
            continue
...
def fsk_tones(x: np.ndarray, fs: float, bw_hint: Optional[float] = None,
    ...
    if symbol_rate and symbol_rate > 0:
        sps = fs / symbol_rate
        if 2.0 <= sps <= 4096.0:
            for w in {1, int(np.clip(round(sps / 4), 1, 16))}:
                f = _smooth(f_raw, w)
                best = None
    ...
    pick = max(valid, key=lambda c: c[0]) if valid else (blanket or (cands[0] if cands else None))
    if pick is None:
    ...
    return {"n_tones": int(n), "tone_freqs": tones.tolist(), "deviation": dev,
            "uniform": bool(pick in valid), "method": method}
...
PROTOCOLS: list[dict] = [
    ...
    dict(name="ADS-B 1090ES (PPM/OOK)", mod=["ook-pulsed"],
         bw=(1.0 * MHz, 8.0 * MHz), rate=(0.9e6, 2.2e6), dur=(50e-6, 130e-6),
