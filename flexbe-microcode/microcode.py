...
def build_program(cfg: bs.BSPNetConfig, hw: bs.HWConfig, inference: int = 0,
    ...
    if brev and cfg.bit_reversal:
        # the write-back occupies the engine read port and the reversal
        # network for N/P cycles per sequence: a datapath pass, not a DMA
        last = add(Descriptor("bs.brev", count=cfg.N),
                   "bs.brev  write-back pass", cycles=brev[0], deps=[last])
...
class RunReport:
    ...
    critical: str = ""
    timeline: Optional[List[Tuple[str, str, float, float, int]]] = None
    #          (name, resource, start_us, end_us, inference)
...
class Sequencer:
    ...
    def run(self, cmds: Sequence[Command], trace: bool = False) -> RunReport:
        p = self.p
        timeline: List[Tuple[str, str, float, float, int]] = []
        free: Dict[str, float] = {}
        busy: Dict[str, float] = {}
        done = [0.0] * len(cmds)
        ...
        for n, c in enumerate(cmds):
            ...
            if trace:
                timeline.append((c.name, res, start, done[n], c.inference))
