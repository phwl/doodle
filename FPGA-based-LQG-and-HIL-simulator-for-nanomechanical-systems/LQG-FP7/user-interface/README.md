...
3. Copy the distribution package `firmware/package/lqg_fp7_package.tar.gz` to the RP via SCP.
4. Extract the package on the RP:

```bash
tar -xf lqg_fp7_package.tar.gz
```

5. Run `./start.sh` to initialize the FPGA bitstream and start the server.
